...
documents = []
with open("../data_acquisition/bm25_documents.txt", "r", encoding="utf-8") as f:
    docs_content = f.read().split("\n\n")  
    documents = [Document(page_content=doc) for doc in docs_content if doc.strip()]

bm25_retriever = BM25Retriever.from_documents(documents, k1=1.5, b=0.75)
...
#rag_responses_list = list(baseline_responses.values())


#baseline_responses_list = list(baseline_responses.values())
...
examples = [
    {
        ...
        "contexts": [doc.page_content for doc in bm25_retriever.invoke(questions[i])]
    }
...
csv_filename = "../evaluation_results/rag_evaluation_results_BM25_easy.csv"
