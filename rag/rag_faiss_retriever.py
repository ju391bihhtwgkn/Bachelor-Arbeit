...
documents = []
with open("../data_acquisition/faiss_documents.txt", "r", encoding="utf-8") as f:
...
faiss_retriever = vectorstore.as_retriever(search_type="similarity_score_threshold", 
...
# Extrahiere Fragen und goldene Antworten
...
examples = [
    {
        ...
        "contexts": [doc.page_content for doc in retriever.invoke(questions[i])]
    }
...
for evaluator in evaluators:
    ...
    # Ergebnisse durchlaufen und ausgeben
        #print(f"Question: {eval_results["question"]}")
        #print(f"Response: {eval_results["answer"]}")
        #print(f"Ground_truth {eval_results["ground_truth"]}")
        #print(f"Score for {evaluator.metric.name}: {eval_results[f"{evaluator.metric.name}"]}")
        #print("\n")
...
csv_filename = "../evaluation_results/rag_evaluation_results_faiss_easy.csv"
...
with open(csv_filename, mode='w', newline='', encoding='utf-8') as file:
    ...
    for i, question in enumerate(questions):
        ...
        # Extrahiere LLM-Bewertungen, wenn vorhanden
        ...
        # Versuche, die Bewertungen (Accuracy, Relevance, Completeness) aus der LLM-Antwort zu extrahieren
